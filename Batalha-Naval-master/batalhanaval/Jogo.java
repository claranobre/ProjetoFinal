...
public class Jogo implements Serializable {
    ...
    public void setEstado(int estado) {
        if (estado == POSICIONANDO_NAVIOS) {
            this.addEvento("Prepare-se! A batalha vai comecar!");
        } else if (estado == TERMINADO) {
            if (this.getVencedor() == jogadores[0])
                addEvento("A batalha terminou! Voce foi derrotado!");
            else
                addEvento("A batalha terminou! Voce venceu!");
        }
     ...
     *
     * @param mensagem de saída dos eventos
     */
